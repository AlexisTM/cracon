...
void test_all_items(nlohmann::json const &json_data, std::string const &name) {
  for (auto &data : json_data.items()) {
    ...
    if (data.key().find(name) != std::string::npos) {
      EXPECT_TRUE(is_similar<T>(data.value()))
          << data.key() << " should be a " << name;
    } else {
      EXPECT_FALSE(is_similar<T>(data.value()))
          << data.key() << " should not be a " << name;
    }
  }
}
...
class IsSimilarTest : public ::testing::Test {
 protected:
  ...
  IsSimilarTest() {
    ...
    json_data_["string_longish"] =
        "The universe is large, I mean really large, you know? ";
    ...
    // Using setters from C++, the values are not identical to the JSON just
    // read equivalent.
...
TEST_F(IsSimilarTest, test_int8) { test_all_items<int8_t>(json_data_, "int8"); }
